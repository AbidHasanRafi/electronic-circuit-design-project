...
const int16_t XP = 7, XM = A1, YP = A2, YM = 6; // 240x320 ID=0x9341
const int16_t TS_LEFT = 918, TS_RT = 201, TS_TOP = 932, TS_BOT = 223;
...
void checkBrickCollision(game_type *game, game_state_type *state, uint16_t x, uint16_t y)
{
  ...
  if (collissions > 0)
  {
    ...
    if ((((x % state->brickwidth) == 0) && (state->velx < 0)) || ((((x + game->ballsize) % state->brickwidth) == 0) && (state->velx > 0)))
    {
...
void checkBorderCollision(game_type *game, game_state_type *state, uint16_t x, uint16_t y)
{
  ...
  if (((y + game->ballsize) >= state->bottom) && ((y + game->ballsize) <= (state->bottom + game->playerheight)) && (x >= state->playerx) && (x <= (state->playerx + game->playerwidth)))
  {
